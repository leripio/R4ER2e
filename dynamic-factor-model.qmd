...
Policymakers and analysts routinely seek to characterize short-term fluctuations in prices as either persistent or temporary. This issue is often addressed using core measures that exclude those items that are the most volatile or most subject to supply shocks from the inflation index.
...
In fact, this idea is represented by a special class of state-space models known as the Dynamic Factor Model (DFM). In short, a DFM allows us to estimate common underlying trends among a relatively large set of time series (see @marss, chapter 10). Using @fed's notation, the model has the following structure:
...
where $\pi_{it}$ is the CPI for each subsector $i$, which is the sum of the common trend, $\chi_{it}$, and the idiosyncratic component, $\xi_{it}$. In state-space language, this is the observation equation, and $\chi_{it}$ is the hidden state ($x_t$). However, in this case, the hidden state follows an autoregressive process of order $p$, with $p$ usually greater than 1. In addition, each subsector will have an individual contribution to the common trend, given by $\lambda_i$ --- also known as the *factor loading*.

## Writing out the DFM as MARSS

We start by writing out the above DFM in MARSS form. The LHS of the observation equation now contains the $k$ time series corresponding to each observation of the common underlying process, all of them are a function of a single hidden process (the common component) plus a Gaussian innovation term.[^dynamic-factor-model-1]

[^dynamic-factor-model-1]: @fed consider the innovation term to follow an AR(1) process. Incorporating this assumption would require an even more complex transition equation.
...
The transition equation, in turn, is a function of its own $p$ lags. In MARSS form, each lag will be represented by a separate variable, although only the first one, $x_t$, will have an autoregressive structure. The others are simply treated as identity processes.
...
For this exercise, we'll use data on the 51 subsectors that make up the Brazilian CPI. All these time series are the monthly percentage change accumulated in the last twelve months. This eliminates the need to add seasonal components to the model.

::: {.callout-tip}
## Data
The data set containing data on the 51 subsectors that make up the Brazilian CPI is available in the `R4ER2data` package under the name `cpi_item_br`.
:::

```{r}
#| warning: false
library(tidyverse)
cpi_item_br <- R4ER2data::cpi_item_br

cpi_twelveMonths <- cpi_item_br$CPI |> 
  arrange(date)
...
model.spec$Z      <- matrix(list(0), nrow(cpi_ts), 4)
