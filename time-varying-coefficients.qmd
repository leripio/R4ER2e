...
For this exercise, we'll use the CPI in Brazil excluding regulated prices such as public transport, energy, and others. After importing the series from the Brazilian Central Bank API with the `rbcb` package, we'll compute the seasonally adjusted series from the original data with the `seasonal` package (see the section on Seasonal Adjustment for details).
