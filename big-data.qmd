...
In a nutshell, DuckDB seamlessly converts your flat file into a database, enabling you to fetch data using SQL queries. Describing it this simply, however, might understate the power of this tool. Three aspects are particularly noteworthy in this process. First, it handles column classes and all the tedious details that often arise when writing raw data to a database with remarkable ease. Second, it's incredibly fast at creating tables and executing queries. Third, it supports a wide range of popular file formats -- including CSV, Parquet, and others -- and is fully multi-platform.
...
```{r}
#| eval: false
dbExecute(con, "CREATE TABLE cps AS SELECT * FROM read_csv('data/cps/*.csv', union_by_name = true, null_padding = true);")
```
