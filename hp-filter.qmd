...
To see how it works in practice, let's use data on Brazilian GDP provided by IBGE. Next, we need to convert it into a `ts` object, since this is the default input format required by `hpfilter` function.
